...
def _read_dataset(hook: SqlHook, run_id: str, filename_pattern: str) -> pd.DataFrame:
    # On sélectionne par chemin (rel_path LIKE) pour cibler le bon fichier
    sql = """
    ...
    # normaliser colonnes: sans accents, lower, trim
    def norm(x: str) -> str:
        x = re.sub(r"[ÀÂÄàáâäÉÈÊËéèêëÍÌÎÏíìîïÓÒÔÖóòôöÚÙÛÜúùûüÇç]", "", x)
...
def load_regions(**ctx):
    ...
    # UPSERT simple: wipe & reload for now (référentiel petit)
...
def load_provinces(**ctx):
    hook = SqlHook(conn_id="mssql_default", schema=STAGING_DB)
    run_id = _latest_run_id(hook)
    df = _read_dataset(hook, run_id, "%provinc%.csv%")  # province.csv
    if df.empty:
...
def load_communes(**ctx):
    hook = SqlHook(conn_id="mssql_default", schema=STAGING_DB)
    run_id = _latest_run_id(hook)
    df = _read_dataset(hook, run_id, "%commune%.csv%")  # Commune (1).csv
    if df.empty:
